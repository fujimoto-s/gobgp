...
func makePathStringForMonitoring(p *table.Path, showIdentifier bgp.BGPAddPathMode) []interface{} {
...
func showRouteForMonitoring(pathList []*table.Path, showIdentifier bgp.BGPAddPathMode) {
	var pathStrs [][]interface{}

	for _, p := range pathList {
		pathStrs = append(pathStrs, makePathStringForMonitoring(p, showIdentifier))
	}
	...
	monitor := func(recver interface {
		...
		for {
			...
			if globalOpts.Json {
				...
				showRouteForMonitoring(dst.GetAllKnownPathList(), showIdentifier)
			}
		}
	}
