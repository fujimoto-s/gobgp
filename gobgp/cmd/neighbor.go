...
func makePathString(p *table.Path, idx int, now time.Time, showAge, showBest, showLabel bool, showIdentifier bgp.BGPAddPathMode) []interface{} {
	nlri := p.GetNlri()
...
func showRoute(destinationList [][]*table.Path, showAge, showBest, showLabel bool, showIdentifier bgp.BGPAddPathMode) {
	...
	for _, pathList := range destinationList {
		for idx, p := range pathList {
			pathStrs = append(pathStrs, makePathString(p, idx, now, showAge, showBest, showLabel, showIdentifier))
		}
	}
...
func showNeighborRib(r string, name string, args []string) error {
	...
	if validationTarget != "" {
		...
		if d == nil {
			return fmt.Errorf("no such route in RIB")
		}
